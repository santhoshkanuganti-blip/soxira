    ...
    <section className="py-8 px-4 sm:px-8">
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-8 md:grid-cols-2 md:items-center">
          ...
          >
            <span className="text-xs font-semibold uppercase tracking-[0.28em] text-accent">Who We Are</span>
            <h2 className="mt-2 text-2xl font-bold text-ink leading-tight sm:text-3xl">
            ...
            <div className="mt-5 grid grid-cols-2 gap-2.5">
              {stats.map((s) => (
                <div key={s.label} className="rounded-2xl border border-line bg-surface-2 p-3 -sm">
          ...
          <motion.div
            ...
            className="relative"
          >
            <div className="rounded-3xl border border-line bg-accent-tint p-5 shadow-2xl sm:p-6">
              <div className="absolute -top-6 -right-6 h-32 w-32 rounded-full bg-accent-tint blur-2xl" />
              <div className="absolute -bottom-6 -left-6 h-28 w-28 rounded-full bg-accent-tint blur-2xl" />
              <div className="relative space-y-3 text-center">
                {['AI-Powered Solutions', 'Cloud Transformation', 'Data Engineering', 'Mobile & Web Apps'].map((item) => (
                  <div key={item} className="rounded-2xl border border-line bg-surface-2 px-4 py-2.5 text-xs font-semibold text-ink shadow-lg sm:text-sm">
                    {item}
                  </div>
                ))}
              </div>
